...
def ventilate():
    ...
    # Load a default patient state
    data_root_dir = "/mnt/data/"
    patient_file = f"{data_root_dir}/states/StandardMale@0s.json"
    print(f"Loading patient from {patient_file}")
    if not pulse.serialize_from_file(patient_file, data_mgr):
        raise Exception("Unable to load initial state file")
